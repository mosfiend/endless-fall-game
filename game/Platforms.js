...
export class PlatBuffer extends PIXI.Container {
  ...
  set(idx, height, clr) {
    ...
    if (!this.#plats[idx]) {
      this.#plats[idx] = new Platform(
        (Math.random() * 0.4 + 0.6) * this.screenWidth,
        height,
        100,
        100,
        clr,
...
export class Platform extends PIXI.Graphics {
  // The physics is implemented into the graphics object
  constructor(x, y, width, height, clr) {
    super();
    //graphics
    this.clr = clr;
    this.beginFill(this.clr).drawRect(0, 0, width, height);
  ...
  move(height, width) {
    Matter.Body.setPosition(this.body, {
      ...
      y: this.body.position.y - this.dy,
    });
    // console.log(this.y)
    if (this.y < 0) {
