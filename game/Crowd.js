...
export class Person extends PIXI.Graphics {
  ...
  constructor(screenWidth, screenHeight, clr, roadHeight) {
    ...
    this.clr = clr;
    this.dimensions = 30;
    this.beginFill(this.clr).drawRect(0, 0, this.dimensions, this.dimensions);
    ...
    this.boundary = this.x
      ? 0
      : this.screenWidth + this.dir * this.screenWidth * Math.random() * 0.5;
    //physics
  ...
  move() {
    ...
    if (
      ((this.x < 0 && this.dir < 0) ||
        (this.x > this.boundary && this.dir > 0)) &&
      !this.panicking
    ) {
      this.reuse();
    }
  ...
  panic() {
    if (!this.panicking) {
      ...
      const eyeL = new PIXI.Graphics()
        .beginFill(0x101022)
        .drawRect(0, 0, this.dimensions / 5, this.dimensions / 5);
